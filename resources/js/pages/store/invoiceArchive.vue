  ...
  <div v-if="isLoadig">
                  ...
                  >
                    ...
                    <template #cell(archived_at)="data">
                      <span class="badge-dark badge-pill">{{
                        handelDate(data.item.deleted_at)
                      }}</span>
                    </template>
    ...
    <b-modal id="invoiceModel2" hide-footer>
      ...
      <b-button
        ...
        v-b-modal.modal-sm
        variant="default"
        @click="$bvModal.hide('invoiceModel2')"
        >{{ $t("Cancel") }}</b-button
      >
      <b-button
        class="mt-3"
...
export default {
  ...
  beforeMount() {
    axios
      .get("product_invoicesOnlyArchived")
      .then((res) => {
  ...
  methods: {
    ...
    restoreInvoice() {
      axios
        .get(`restoreInvoiceInvoice/${this.id2}`)
        .then((res) => {
          if (res.data.success) {
    ...
    deletInvoice() {
      axios
        .get(`deleteThisInvoice/${this.id2}`)
        .then((res) => {
          if (res.data.success) {
            this.product_invoices_archived.splice(this.key2, 1);
            this.id2 = "";
