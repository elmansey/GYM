              ...
              <div class="form-row">
                  ...
                  >
                    ...
                    >
                      <template v-slot:option="option">
                        <option :disabled="IsSet(option.id)">
                          {{ option.name }}
                        </option>
                      </template>
                    </v-select>
                ...
                <div class="form-group col-sm-3">
                  ...
                  <input
                    ...
                    v-model="product_quantity[index]"
                  />
                  <div class="invalid-feedback"></div>
                </div>
                ...
                <div class="form-group col-sm-1">
                  <feather
                    v-if="index > 0"
                    @click.prevent="deleteRecourd(index)"
                    type="trash"
                    style="cursor: pointer; margin-top: 45px"
                  ></feather>
                </div>
              </div>
            </div>
              ...
              <div class="checkout-details">
                <div class="order-box">
                  <div class="table-responsive invoice-table" id="table">
                    ...
                    <div>
                      <button
                        @click.prevent="handelSaveInvoice"
              ...
              <div class="row">
                <div class="col-sm-6">
                  <div class="media">
                    <div class="media-body m-l-20">
                      <h4 class="media-heading" style="margin-top: 45px">
                        {{ $store.getters.SETTINGS.Gym_name }}
                      </h4>
                      <p>
                        <span>
                          {{ $t("seller name :") }}
                          {{ $store.getters.USER.name }} <br />
                          {{ $t("seller RF code :") }}
                          {{ $store.getters.USER.RF_code }}
                        </span>
                      </p>
                    </div>
                  </div>
                ...
                <div class="col-sm-6">
                  <div class="text-md-right">
                    ...
                    <P class="digits counter">{{
                      InvoiceAfterAdd.invoice_number
                    }}</P>
                    <p>
                      {{ $t("date") }}:
            ...
            <div>
              <div class="table-responsive invoice-table" id="table">
                <table class="table table-bordered table-striped">
                  ...
                  <tbody>
                    <tr
                      v-for="(item, index) in InvoiceAfterAdd.invoice_details"
                      :key="index"
                    >
                      <td class="item">
                ...
                <div class="order-box">
                  ...
                  <ul class="sub-total">
                    <li>
                      {{ $t("Subtotal") }}
                      <span class="count"
                        >LE {{ InvoiceAfterAdd.invoice_total }}</span
                      >
                    </li>
                  </ul>
                  <ul class="sub-total">
  ...
  methods: {
    ...
    getRecourdTotalAndUnitPrice(index) {
      ...
      if (
        ...
        this.product_name[index] == null ||
        this.product_name[index] == undefined ||
        !this.product_name[index]
      ) {
        this.product_name.map((v, k) => {
          if (k == index) {
      ...
      if (this.product_quantity[index]) {
        this.recourd_total[index] =
          Number(this.unit_price[index]) * Number(this.product_quantity[index]);
      }
      ...
      if (this.product_quantity[index] && this.product_name[index]) {
        var recourd = {
          ...
          product_quantity: this.product_quantity[index],
          recourd_total: this.recourd_total[index],
        };
    ...
    handelSaveInvoice() {
      if (this.product_name.length > 0) {
        ...
        axios
          .post("add_product_invoice", formData)
          .then((res) => {
            if (res.data.success) {
