<template>
  <div v-if="isLoading">
    <Breadcrumbs
      :main="$t('Dashboard')"
      :title="edit ? $t('edit product') : $t('add product')"
    />
                ...
                <form @submit.prevent="handelSubmitProductForm" id="form">
                  <div class="row">
                    <div class="mb-2 col-md-6 col-lg-4 col-sm-12">
                      ...
                      <input
                        name="name"
                        v-model="product_info.product_name"
                        :class="[
                          'form-control',
                      ...
                      />
                      <small style="color: red" v-if="error.product_name">{{
                        $t(error.product_name[0])
                      }}</small>
                    </div>
                    <div class="mb-2 col-md-6 col-lg-4 col-sm-12">
                      ...
                      <input
                        name="product_price"
                        type="number"
                        v-model="product_info.product_price"
                        :class="[
                          'form-control',
                          error.product_price ? 'is-invalid' : '',
                        ]"
                        :placeholder="$t('product price')"
                      />
                      <small style="color: red" v-if="error.product_price">{{
                        $t(error.product_price[0])
                      }}</small>
                    </div>
                    <div class="mb-2 col-md-6 col-lg-4 col-sm-12">
                      <div class="col-form-label">
                      ...
                      <input
                        ...
                        type="number"
                        v-model="product_info.product_quantity"
                        :class="[
                          'form-control',
                          error.Product_Quantity ? 'is-invalid' : '',
                        ]"
                        :placeholder="$t('Product Quantity')"
                      />
                      <small style="color: red" v-if="error.Product_Quantity">{{
                        $t(error.Product_Quantity[0])
                      }}</small>
                    </div>
                    ...
                    <div class="mb-2 col-md-12 col-lg-12 col-sm-12">
                      ...
                      <small style="color: red" v-if="error.product_img">{{
                        $t(error.product_img[0])
                      }}</small>
                    </div>
                    ...
                    <div class="mb-2 col-md-6 col-lg-6 col-sm-12">
                      ...
                      <input
                        ...
                        type="date"
                        v-model="product_info.Production_Date"
                        :class="[
                          'form-control',
                          error.Production_Date ? 'is-invalid' : '',
                        ]"
                      />
                      <small style="color: red" v-if="error.Production_Date">{{
                        $t(error.Production_Date[0])
                      }}</small>
                    </div>
                    <div class="mb-2 col-md-6 col-lg-6 col-sm-12">
                      ...
                      <input
                        name="Expiry_date"
                        type="date"
                        v-model="product_info.Expiry_date"
                        :class="[
                          'form-control',
                          error.Expiry_date ? 'is-invalid' : '',
                        ]"
                      />
                      <small style="color: red" v-if="error.Expiry_date">{{
                        $t(error.Expiry_date[0])
                      }}</small>
                    </div>
                  </div>
...
export default {
  ...
  beforeMount() {
    if (this.$route.params.productId) {
      ...
      axios
        .get(`getProductById/${this.$route.params.productId}`)
        .then((res) => {
          if (res.data.success) {
            (this.product_info.product_name = res.data.product.product_name),
              (this.product_info.product_price =
                res.data.product.product_price),
              (this.product_info.product_quantity =
                res.data.product.Product_Quantity),
              (this.product_info.Production_Date =
                res.data.product.Production_Date),
  ...
  methods: {
    handelSubmitProductForm() {
      if (this.edit == false) {
        ...
        axios
          .post("add_product", formData, config)
          .then((res) => {
            if (res.data.success) {
              this.error = "";
              (this.product_info = {
        ...
        axios
          .post(
            `edit_product/${this.$route.params.productId}`,
            formData,
            config
          )
          .then((res) => {
