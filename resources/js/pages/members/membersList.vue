            ...
            <div class="card-body">
                  ...
                  <div class="table-responsive vue-smart">
                    ...
                    >
                      ...
                      <template #cell(name)="data">
                        <router-link
                          v-if="can('show-member-details')"
                          :to="{
                            name: 'showMemberDetails',
                            params: { memberBaseId: data.item.id },
                          }"
                        >
                          {{ data.item.name }}
                        </router-link>
                      ...
                      <template #cell(action)="data">
                        ...
                        >
                          <b-dropdown-item
                            v-if="can('freezeMemberAccount')"
                            @click.prevent="
                              freezeAccountHandel(data.item.id, data.index)
                            "
                          >
                            <feather style="width: 15px" type="zap"></feather>
                            {{ $t("freeze account") }}
                          </b-dropdown-item>
                          <b-dropdown-item>
                            <router-link
                              :to="{
                                name: 'editMember',
                                params: { memberId: data.item.id },
                              }"
                              v-if="can('edit-member-in-gym')"
                            >
                              <feather
                                style="width: 15px"
                                type="edit-2"
                          ...
                          <b-dropdown-item
                            ...
                            v-if="can('delete-member-in-gym')"
                            ><feather style="width: 15px" type="trash"></feather
              ...
              <b-modal id="freeze" hide-footer>
                ...
                <div class="d-block text-center">
                  <h5>
                    <i class="icofont icofont-snow-alt"></i>
                    {{ $t("are you sure to freeze this account") }}
                  </h5>
                </div>
                <b-button
                  class="mt-3"
...
export default {
  data() {
    return {
      ...
      tablefields1: [
        ...
        {
          key: "period_Expiry",
          label: this.$t("period Expiry"),
          sortable: false,
        },
        {
          key: "member_ships_relation.name",
  ...
  methods: {
    ...
    freezeAccount() {
      axios
        .get(`freezeThisAccount/${this.freezeAccountId}`)
        .then((res) => {
          if (res.data.success) {
