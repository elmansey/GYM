          ...
          <div class="card">
            <div class="card-body">
                    ...
                    >
                      ...
                      <template #cell(freeze_in)="data">
                        <span class="badge badge-pill badge-primary">
                          {{ data.item.Account_freeze }}
                        </span>
                      </template>

                      <template #cell(days_left_before_freezing)="data">
                        <span class="badge badge-pill badge-primary">
                          {{ data.item.days_left_before_freezing }}
                        </span>
                      </template>
                      ...
                      <template #cell(action)="data">
                        ...
                        >
                          <b-dropdown-item
                            @click.prevent="
                              unfreezeHandel(data.item.id, data.index)
                            "
                          >
              ...
              <b-modal id="unfreeze" hide-footer>
                <template #modal-title> unfreeze account </template>
                <div class="d-block text-center">
                  <h5>
                    <i class="icofont icofont-snow-alt"></i> are you sure to
                ...
                <b-button
                  ...
                  v-b-modal.modal-sm
                  variant="dark"
                  @click.prevent="unfreezeThisAccount"
                  >unfreeze</b-button
                >
              </b-modal>
            </div>
...
export default {
  ...
  methods: {
    unfreezeThisAccount() {
      axios
        .get(`unFreezeThisAccount/${this.unFreezeAccountId}`)
        .then((res) => {
          if (res.data.success) {
