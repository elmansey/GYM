                            ...
                            <form @submit.prevent="submitNewPasswordForm">
                                ...
                                <input type="hidden"   v-model="user.newPassword.token"/>
                                    <div class="form-group">
                                        ...
                                        <b-form-invalid-feedback style="color:red" v-if="(   click3 && !$v.user.newPassword.password.maxLength )" > the  max length character is {{ $v.user.newPassword.password.$params.maxLength.max }}</b-form-invalid-feedback>
                                        <b-form-invalid-feedback style="color:red" v-if="( click3 &&!$v.user.newPassword.password.minLength )" > the min lenght character is {{ $v.user.newPassword.password.$params.minLength.min }} </b-form-invalid-feedback>
            ...
            .then(res => {
                ...
                if(res.data.success){
                     ...
                     this.$router.push({name : 'login'});
                          Toast.fire({
                                icon: 'success',
                                title: 'password reseted successfully login with new password now'
                            })
