<template>
  <div v-if="isLoading">
    <Breadcrumbs
      :main="$t('Dashboard')"
      :title="edit ? $t('edit admin') : $t('add admin')"
    />
                ...
                <form @submit.prevent="handelSubmitData" id="myform">
                  <div class="row">
                    ...
                    <div class="mb-2 col-md-6 col-lg-6 col-sm-12">
                      ...
                      <small style="color: red" v-if="error.role">{{
                        $t(error.role[0])
                      }}</small>
                    </div>
...
export default {
  data() {
    return {
      ...
      singledropzoneOptions: {
        ...
        maxNumberOfFiles: 1,
        addRemoveLinks: true,
        dictDefaultMessage: `<i class='icon-cloud-up'></i><br />Select Profile Picture`,
      },
    };
  },
  components: {
  ...
  beforeMount() {
    if (this.$route.params.adminId) {
      this.edit = true;
      axios
        .get(`getUserById/${this.$route.params.adminId}`)
  ...
  methods: {
    handelSubmitData() {
      ...
      if (this.edit) {
        ...
        axios
          .post(`userUpdate/${this.$route.params.adminId}`, formData, config)
          .then((res) => {
            if (res.data.success == true) {
    ...
    editAdmin() {
      axios
        .post("userUpdate", this.adminData)
        .then((res) => {
          if (res.data.success == true) {
            Toast.fire({
              icon: "success",
